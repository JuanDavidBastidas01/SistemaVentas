...
{
	$alert='';
	if(empty($_POST['proveedor']) || empty($_POST['producto']) || empty($_POST['precio']) || $_POST['id'] <= 0 || empty($_POST['foto_actual'] || $_POST['foto_remove']))
	{
		...
		if($nombre_foto != '')
		{
			...
			$query_update = mysqli_query($conection,"UPDATE producto
															SET descripcion = '$producto',
																...
																precio = $precio,
																foto = '$imgProducto'
																WHERE codproducto = $codproducto"); //16:54
...
//VALIAR PRODUCTO
	if(empty($_REQUEST['id'])){
		...
		$query_producto = mysqli_query($conection,"SELECT p.codproducto,p.descripcion,p.precio,p.foto,pr.codproveedor,pr.proveedor
			...
			WHERE p.codproducto = $id_prducto 
			AND p.estatus = 1");
...
<head>
	...
	<title>Actualizar Producto | Fruver</title>
</head>
<body>
	...
	<section id="container">
		<div class="form_register">
			...
			<form action="" method="post" enctype="multipart/form-data">
				...
				<input type="submit" value="Actualizar Producto" class="btn_save"></form>
		</div>
	</section>
